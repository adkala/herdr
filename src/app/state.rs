...
pub(crate) struct PopupPaneState {
    ...
    pub chrome: crate::popup_size::PopupChrome,
}
...
pub struct AppState {
    ...
    /// Draw only shared split dividers, no outer pane frame (tmux style).
    /// Only meaningful while `pane_borders` is true; implies shared dividers,
