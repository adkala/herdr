...
mod tests {
    ...
    fn legacy_reader_extends_incomplete_mouse_timeouts() {
        ...
        assert_eq!(
            input_flush_timeout_ms(&sgr_mouse, RAW_INPUT_IDLE_FLUSH_TIMEOUT_MS),
            MOUSE_ACTIVE_ESCAPE_SEQUENCE_FLUSH_TIMEOUT_MS
        );
        // escape-time=0 must not shorten mouse-sequence reassembly.
        assert_eq!(
            input_flush_timeout_ms(&sgr_mouse, 0),
            MOUSE_ACTIVE_ESCAPE_SEQUENCE_FLUSH_TIMEOUT_MS
        );
        ...
        for split in 3..report.len() {
            ...
            assert_eq!(
                input_flush_timeout_ms(&default_mouse, RAW_INPUT_IDLE_FLUSH_TIMEOUT_MS),
                MOUSE_ACTIVE_ESCAPE_SEQUENCE_FLUSH_TIMEOUT_MS
            );
            assert!(matches!(
