...
/// Wire variant index of `ServerMessage::ClipboardQuery`. bincode numbers enum
/// variants by declaration order, and this integration test cannot name the
/// enum, so `protocol::wire::tests::server_clipboard_query_wire_variant` pins
/// the same value; update both when upstream appends variants ahead of it.
const SERVER_CLIPBOARD_QUERY_VARIANT: u32 = 15;
...
fn osc52_terminal_paste_round_trip_reaches_pane() {
    ...
    let got_query = wait_for_message_variant(
        ...
        SERVER_CLIPBOARD_QUERY_VARIANT,
    )
