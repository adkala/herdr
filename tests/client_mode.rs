...
fn osc52_terminal_paste_round_trip_reaches_pane() {
    ...
    assert!(error.is_none(), "{:?}", error);
    wait_for_client_shell_bootstrap(&mut stream, Duration::from_secs(10))
        .expect("client shell should bootstrap");
    send_client_shell_focus(&mut stream, true).expect("focus client shell");
    ...
    // In the pane's shell: switch the pane tty to raw so the reply is
