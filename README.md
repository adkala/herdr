...
| attached clients render colored underlines: the in-memory `CellData` carries the SGR 58 underline color and the client emits `58:2::r:g:b` / `58:5:n`, so Neovim's red diagnostic undercurls stay red through herdr (they fell back to the text color). The color travels in a negotiated `shell.surface.v2` surface codec (appended `PaneSurfaceV2` / `PaneSurfacePatchV2` variants, wire tags 21 and 22); `shell.surface.v1` clients keep getting byte-identical v1 frames through the frozen mirrors in `src/protocol/surface_v1.rs` | `628ecad`, `a7f1918` (codec negotiation) | `pr/underline-color` | PR candidate; upstream bug (#1252, #1169, #1178 are still open), fork carries it until merged. The client hello offers `["shell.surface.v2", "shell.surface.v1"]` and the server answers with the first codec it speaks, so a fork client on an upstream server (or a stock 0.9.0 client on a fork server) falls back to v1 cleanly; the upstream generation-1 digests are back to their frozen values and the private protocol stays at 22. Fork builds from before `a7f1918` spoke a widened v1 on tag 13 with protocol 23, so upgrade client and server together once more when moving off them. The branch is one squashed commit (re-cut 2026-09-13) |
...
| workspace id length test no longer depends on how many workspaces earlier tests allocated from the global counter | `pr/workspace-id-test-isolation` | test-only; came off with `pr/popup-geometry-defaults`, the only thing that needed it. Re-cut onto upstream `d184b41` on 2026-09-13 (`c01d371`); upstream still has the `first.len() <= 3` assertion it replaces |
| Kitty graphics for clients attached from inside tmux: images go through `DCS tmux;` passthrough as unicode-placeholder virtual placements | `pr/kitty-tmux-placeholders` | **in progress, never on `master`**; Addison's WIP, based on upstream `c2637dc1` and bumping the protocol to 22, which upstream has since used itself. A plain rebase onto `d184b41` conflicts in 10 files (31 hunks): upstream #3487 deleted the in-process `App::run` loop and the server-side `encode_local_pane_graphics` / `ClientLaunchMode` paths the branch patches, and graphics now travel to the client shell as `SurfaceGraphicsScene` assets and placements. It needs a redesign on the client-shell graphics path (placeholder cells in the shell's composition, passthrough wrapping where the shell writes graphics), not a conflict resolution, so it was left untouched at `a4d5acd2` and pushed to `origin` as a backup on 2026-09-13 |
...
a plain cherry-pick; `pr/sidebar-tab-label` stacks on `pr/auto-tab-titles`, and
`pr/underline-color` was re-cut again on 2026-09-13 as one squashed commit that
carries the `shell.surface.v2` negotiation without the OSC 52 changes it shares
plumbing with on `master`. `pr/workspace-id-test-isolation` was re-cut onto
`d184b41` the same day. The remaining branches in the table below
(`pr/popup-geometry-defaults`, `pr/outer-pane-borders`,
`pr/split-only-pane-borders`, `fork/hdev-config-overlay`) stay on their older
bases, and `pr/kitty-tmux-placeholders` stays on `c2637dc1` until it is
redesigned.
