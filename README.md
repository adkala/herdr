...
| popup `chrome = "modal"`: popup panes (`plugin.pane.open`, `herdr plugin pane open --chrome`, `type = "popup"` keybinds) can render with the built-in settings overlay treatment — dimmed backdrop, panel shell, bold title header row; popup keybinds title the header from `description` | `bfd0653f`, `2fd7623b` | `pr/popup-modal-chrome` | PR candidate |
