...
main() {
    ...
    if [ "$os" = "linux" ]; then
        # Linux installs this fork's own build from the rolling `dev` GitHub
        # release on adkala/herdr, not the upstream herdr.dev manifest. That
        # release ships raw binaries with no sha256 manifest, so the checksum
        # step is skipped for this path (transport integrity relies on HTTPS).
        URL="https://github.com/adkala/herdr/releases/download/dev/herdr-${TARGET}"
        VERSION=""
        VERIFY_CHECKSUM=0
        log "installing fork dev build for ${TARGET}"
    else
        VERIFY_CHECKSUM=1
