...
If you SSH into the server first and run `herdr` there, Herdr runs entirely on the server. That mode is useful and simple. Copies still reach your local clipboard through OSC 52 writes, and setting `advanced.osc52_paste = "terminal"` on the server lets OSC 52-aware pane applications (such as Neovim) paste from your local clipboard too, when your terminal supports OSC 52 reads. Richer local desktop features, such as image clipboard paste, still need `herdr --remote`.
