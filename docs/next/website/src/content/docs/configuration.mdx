...
Tabs that were not renamed are labeled with their number. Set `tab_titles = "terminal_title"` under `[ui]` to label them with their focused pane's terminal title instead, like tmux `automatic-rename` — the label follows whatever the program in the pane last set through `OSC 0`/`OSC 2`, with agent spinner frames stripped, and falls back to the tab number while no title is set. Renamed tabs always keep their custom name, and long inherited titles are truncated so one verbose title cannot push the other tabs out of the row. The same label shows in the mobile layout, the navigator, the `{tab}` window-title token, `herdr tab list`, and the sidebar's agent `tab` token, which an inherited title also fills in a single-tab workspace.
...
- `tab` — tab name when available: a custom name, or the title an auto-named tab inherits under `ui.tab_titles = "terminal_title"`. A bare tab number only shows in multi-tab workspaces.
...
Pane applications that read the clipboard with OSC 52 (for example Neovim paste) get no answer by default, because answering lets any process in a pane read the clipboard invisibly. Opt in with `advanced.osc52_paste`:
...
`true` (or `"server"`) answers with the clipboard of the machine running the Herdr server. `"terminal"` forwards the query to the terminal Herdr is displayed in and relays its reply, so panes paste from your local clipboard even when Herdr runs on an SSH remote; your terminal must support OSC 52 reads and may ask for permission, and panes receive an empty reply after a 5 second timeout. OSC 52 clipboard writes (copy) always work and are unaffected by this key.
...
A client started inside a tmux pane (`TMUX` is set) cannot place images at cursor positions: tmux only relays Kitty commands through `DCS tmux;` passthrough and never tells the outer terminal where the pane's cursor is. Herdr detects tmux automatically and switches to Kitty [Unicode placeholders](https://sw.kovidgoyal.net/kitty/graphics-protocol/#unicode-placeholders): each image is uploaded as a virtual placement, and the pane cells it covers are drawn as `U+10EEEE` placeholder characters that carry the image and placement ids in their colors. tmux forwards those like ordinary text and the outer terminal replaces them with the image. This needs:

- tmux 3.3 or newer with `set -g allow-passthrough on` (or `all`) in `tmux.conf`.
- `set -g focus-events on`. With `allow-passthrough on`, tmux drops passthrough written while the pane is not visible, so Herdr re-uploads every image when its window regains focus; the focus event is what triggers that. `allow-passthrough all` forwards commands while hidden and avoids the re-upload.
- An outer terminal that supports Unicode placeholders, such as kitty or Ghostty.

Placeholder cells replace the text under an image, so inside tmux images placed behind text (a negative Kitty z-index) draw over it instead. `terminal.kitty_graphics = false` disables the tmux path together with the rest of the graphics support, and clients outside tmux keep writing cursor-positioned placements exactly as before.
